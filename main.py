...
All problems solved:
1. Group spam multiple messages working
2. Auto Reply Start/Stop All with Back button
3. Typing delay before auto reply (seen delay → typing → send)
...
async def send_with_typing(client, chat_id, message_text, seen_delay=None, typing_duration=None):
    """Send message with typing effect: seen delay -> typing indicator -> send"""
    
    if seen_delay is None:
    ...
    # Step 1: Seen delay (wait before showing typing)
    if seen_delay > 0:
...
async def start_account(acc):
    try:
        ...
        if custom_msgs:
            ...
            # FIX: Diverse spam messages that actually work
        ...
        while not account_stop_flags.get(acc_id, False):
            ...
            for group in groups[:max_batch]:
                if account_stop_flags.get(acc_id, False) or not account_spam_active.get(acc_id, True):
                    break
                try:
                    # FIX: Rotate through all messages properly
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main":
        ...
        typing_enabled = get_setting('typing_enabled', True)
        set_setting('typing_enabled', not typing_enabled)
        await query.edit_message_text(f"✅ Typing Effect is now {'🟢 ON' if not typing_enabled else '🔴 OFF'}!")
        await asyncio.sleep(1)
        await handle_callback(update, context)
        ...
        await query.edit_message_text(
            ...
            "All accounts will now auto-reply to messages.\n\n"
        ...
        await query.edit_message_text(
            ...
            f"Enter new duration (1-5 seconds):",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="m_ar")]])
        )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main":
        ...
        kb = [[InlineKeyboardButton(f"🗑️ {m['text'][:30]}", callback_data=f"gsmd_{m['id']}")] for m in msgs[:10]]
        kb.append([InlineKeyboardButton("🔙 Back", callback_data="gs_msg")])
        await query.edit_message_text("Select to delete:", reply_markup=Inline
        elif data == "gs_msg_list":
        msgs = load_spam_messages()
        txt = "📋 **All Spam Messages**\n\n"
        if msgs:
