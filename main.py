...
async def txt_h(upd:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    # ─── ACCOUNT: Session String ───
        ...
        ss=txt.strip()
        await upd.message.reply_text("⏳ Verifying...")
        ok,name,uid,phone=await test_session(ss)
        if ok:
            info={'id':gen_acc_id(),'user_id':uid,'name':name,'phone':phone,'session':ss,'api_id':DEFAULT_API_ID,'api_hash':DEFAULT_API_HASH,'enabled':True,'mode':'ai','spam_active':False,'proxy':None,'is_backup':False,'added_at':datetime.now().isoformat()}
            add_account_data(info)
            c2=await start_account(info)
            if c2:
                active_accounts.append(info); account_clients[info['id']]=c2
                account_stats[info['id']]={'auto_sent':0,'spam_sent':0,'running':False,'spam_running':False}
                account_stop_flags[info['id']]=False; register_ar(c2,info)
            await upd.message.reply_text(f"✅ **Added!** 🎉\n👤 {name}\n📱 {phone}\n🆔 `{info['id']}`",parse_mode='Markdown',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="m_acc")]]))
        else: await upd.message.reply_text(f"❌ Failed!\n{name}",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="m_acc")]]))
        ctx.user_data['await']=None
    ...
    # ─── BACKUP ───
        ...
        if ok:
            ...
            await upd.message.reply_text(f"✅ **Backup Added!** 🎉\n👤 {name}\n📱 {phone}\n\nAuto-used when main gets banned.",parse_mode='Markdown',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="ac_bk")]]))
        else: await upd.message.reply_text(f"❌ Failed!\n{name}",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="ac_bk")]]))
        ctx.user_data['await']=None
    
    # ─── PROXY ───
    elif aw=='proxy':
        aid=ctx.user_data.get('pr_aid','')
        if txt.lower()=='remove':
            a=find_account(aid)
            if a: a['proxy']=None
            for x in active_accounts:
                if x['id']==aid: x['proxy']=None
            await upd.message.reply_text("✅ Proxy cleared!",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="ac_pr")]]))
        else:
            try:
                parts=[p.strip() for p in txt.split(':')]
                if len(parts)>=3:
                    pr={'type':parts[0],'addr':parts[1],'port':int(parts[2]),'rdns':True,'username':parts[3] if len(parts)>3 else '','password':parts[4] if len(parts)>4 else ''}
                    a=find_account(aid)
                    if a: a['proxy']=pr
                    for x in active_accounts:
                        if x['id']==aid: x['proxy']=pr
                    await upd.message.reply_text(f"✅ Proxy set!\n{parts[0]}:{parts[1]}:{parts[2]}",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="ac_pr")]]))
                else: await upd.message.reply_text("❌ Format: type:ip:port:user:pass")
            except: await upd.message.reply_text("❌ Invalid format!")
        ctx.user_data['await']=None

# ─── CALLBACK FOR REPLY TYPE ───
async def cb_rt(upd:Update,ctx:ContextTypes.DEFAULT_TYPE):
    q=upd.callback_query; await q.answer()
    if q.data=="rt_exact": ctx.user_data['rt']='exact'
    elif q.data=="rt_cont": ctx.user_data['rt']='contains'
    else: return
    ctx.user_data['await']='rt'
    await q.edit_message_text("➕ **Enter reply text:**",parse_mode='Markdown',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙",callback_data="ar_rp")]]))

# ═══════════════════════════════════════════
# 🌐 FLASK WEB SERVER & MAIN BOOT
# ═══════════════════════════════════════════

@flask_app.route('/')
def home():
    return jsonify({"status":"running","bot":"Unified Bot","accounts":len(active_accounts),"spam":sum(1 for a in active_accounts if account_stats.get(a['id'],{}).get('spam_running',False))})
...
def webhook():
    global ptb_app, event_loop
    if ptb_app is None or event_loop is None: return "Not ready", 503
    try:
        data=request.get_json(force=True)
        update=Update.de_json(data,ptb_app.bot)
        fut=asyncio.run_coroutine_threadsafe(ptb_app.process_update(update),event_loop)
...
async def setup_webhook():
    ...
    async with httpx.AsyncClient(timeout=30) as c:
        await c.post(f"https://api.telegram.org/bot{BOT_TOKEN}/deleteWebhook",json={"drop_pending_updates":True})
        await asyncio.sleep(2)
        r=await c.post(f"https://api.telegram.org/bot{BOT_TOKEN}/setWebhook",json={"url":WEBHOOK_URL,"drop_pending_updates":True,"max_connections":1,"allowed_updates":["message","callback_query"]})
    ...
    while True:
        try:
            for a in active_accounts:
                ...
                if aid in account_clients:
                    try:
                        c=account_clients[aid]
                        if not c.is_connected():
                            await c.connect()
                            if not await c.is_user_authorized(): await c.start()
                    except: pass
            await asyncio.sleep(30)
        except: await asyncio.sleep(30)

async def main():
    global ptb_app, event_loop, bot_ready
    ...
    if os.path.exists(LOCK_FILE):
        ...
        if old and old.isdigit():
            try: os.kill(int(old),0); logger.warning(f"Already running! PID {old}"); return
            except: pass
    ...
    if RENDER_URL:
        ...
        if not ok: logger.error("Webhook failed!")
    ...
    ptb_app.add_handler(CallbackQueryHandler(cb,pattern="^(?!rt_)"))
    ptb_app.add_handler(CallbackQueryHandler(cb_rt,pattern="^(rt_)"))
    ptb_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,txt_h))
    ...
    if RENDER_URL:
        ...
        finally: await ptb_app.stop(); await ptb_app.shutdown()
    else:
        await ptb_app.updater.start_polling(drop_pending_updates=True)
        logger.info("✅ Polling mode!")
        try: await asyncio.Event().wait()
        finally: await ptb_app.updater.stop(); await ptb_app.stop(); await ptb_app.shutdown()
