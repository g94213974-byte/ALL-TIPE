...
All problems solved:
1. Auto Reply + Group Spam ON/OFF buttons with Start All / Stop All
...
8. Emoji added throughout bot for professional look
"""
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main":
        ...
        await query.edit_message_text("✅ **Auto Reply Started for All Accounts!**\n\nAll accounts will now auto-reply to messages.", parse_mode='Markdown')
        ...
        await query.edit_message_text("⏹️ **Auto Reply Stopped for All Accounts!**\n\nNo accounts will auto-reply now.", parse_mode='Markdown')
        await asyncio.sleep(1.5)
        await handle_callback(update, context)
        ...
        start_spam()
        await query.edit_message_text("▶️ **Started All!**\n\nGroup spam is now running on all accounts.", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="m_gs")]]))
    
    elif data == "gs_off":
        stop_spam()
        await query.edit_message_text("⏹️ **Stopped All!**\n\nGroup spam stopped on all accounts.", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="m_gs")]]))
        ...
        if not msgs:
            ...
            return
        kb = [[InlineKeyboardButton(f"🗑️ {m['text'][:30]}", callback_data=f"gsmd_{m['id']}")] for m in msgs[:10]]
